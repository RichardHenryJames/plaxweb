...
/* ---------------------------------------------------- demo badge + routing */
await page.goto(`${BASE}/web/salon`, { waitUntil: 'networkidle' });
...
check('badge quote link carries demo + view', (await quote.getAttribute('href')) === '/web/contact?demo=salon&view=desktop');
...
await page.goto(`${BASE}/web/restaurant`, { waitUntil: 'networkidle' });
...
/* -------------------------------------------------------- EMI calculator */
await page.goto(`${BASE}/web/realestate`, { waitUntil: 'networkidle' });
...
/* ------------------------------------------------------ boutique catalogue */
await page.goto(`${BASE}/web/boutique`, { waitUntil: 'networkidle' });
...
/* --------------------------------------------------------------- lead form */
await page.goto(`${BASE}/web/contact?demo=travel`, { waitUntil: 'networkidle' });
...
for (const path of ['/sitemap.xml', '/robots.txt']) {
  ...
  if (path === '/sitemap.xml') check('sitemap lists all ten demos', (body.match(/<loc>/g) ?? []).length === 12);
}
